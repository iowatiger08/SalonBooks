...
public class OrderServiceImpl extends BaseServiceImpl implements OrderService {
    ...
    private boolean orderHasMatchingOrderItem(Order order, OrderItem orderItem) {
        if (CollectionUtils.isNotEmpty(order.getOrderItems())) {
            for (OrderItem oi : order.getOrderItems()) {
                if (oi.getItem().equals(orderItem.getItem()) && oi.getOrder().equals(orderItem.getOrder())) return true;
            }
        }
        return false;
    }
    ...
    private BigDecimal updateSubTotal(BigDecimal current, BigDecimal itemPrice, BigDecimal itemQuantity) {
    ...
    private BigDecimal updateSubTotal(Set<OrderItem> orderItems) {
        ...
        for (OrderItem oi : orderItems) {
            total = total.add(oi.getItem().getPrice().multiply(oi.getQuantity())).setScale(2, BigDecimal.ROUND_DOWN);
        }
        return total;
    }
